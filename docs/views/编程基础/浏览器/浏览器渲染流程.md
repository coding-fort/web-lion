...
比如 _display:none_ 的节点没有几何信息，因此不会生成到布局树；
...
此外，某些 JavaScript 操作也会触发 reflow，比如获取一些涉及布局信息的属性值（例如`offsetWidth`, `offsetHeight`, `getComputedStyle()`等），这是因为浏览器需要确保它返回的是最新的值。
